...
const mitraController = {
  ...
  getMitraProfile: async (req, res) => {
    try {
      ...
      const mitraData = await db.oneOrNone(
        `
        SELECT m.mitra_id, m.mitra_name, mt.type as type, m.user_id, u.fullname, u.phone_number, u.email,
  ...
  getMitraProfileWithUserId: async (req, res) => {
    try {
      ...
      const mitraData = await db.oneOrNone(
        `
        SELECT m.mitra_id, m.mitra_name, m.bank_name, m.bank_number, mt.mitra_type_id as type_id, mt.type as type, m.user_id, u.fullname, u.phone_number, u.email, 
        ...
        LEFT JOIN images i on ui.image_id = i.image_id
        WHERE m.user_id = $1
        GROUP BY m.mitra_id, u.fullname, u.phone_number, u.email, mt.type, mt.mitra_type_id
    `,
        [userId]
      );
      res.status(200).json({
